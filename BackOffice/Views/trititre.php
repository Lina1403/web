<?php
    include "../Controller/Quiz.php";
    $QuizController=new QuizController();
    $Liste=$QuizController->afficherquiztri();
    $bdd=new PDO('mysql:host=localhost;dbname=gestion_des_cours', 'root', '',);
$Liste = $bdd->query('SELECT * FROM quiz ORDER BY titre');
if (isset ($_GET['s']) AND !empty($_GET['s'])){
    $recherche =    htmlspecialchars($_GET['s']);
  $Liste = $bdd->query('SELECT * FROM quiz WHERE titre  LIKE "%' .$recherche .'%" '  ); 
            ...
            <div class="animated fadeIn">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <thead>
                                ...
                                <tr>
                                                    ...
                                                    <td><?php echo $Quiz["reponses"];?></td>  
                                                    <td><?php echo $Quiz["id_cours"];?></td>
                                                   
 
                                                    <td> <a href="supprimerquiz.php?rep=<?php echo $ligne['id'];?>"><button type="button" class="btn btn-outline-danger">Supprimer</button></a></td>
                                                      <td> <a href="modquiz.php?id=<?=$ligne["id"]?>"><button type="button" class="btn btn-outline-info">modifier</button></a></td>         
                        ...
                        <form method="GET">
                <input type="search" name="s" placeholder="rechercher un titre" >
                 <input class="btn btn-outline-danger" type="submit" name="envoyer" >
                 </form>
                  <section > 
                    <?PHP if($Liste->rowCount()>0)
                   { while($Quiz =$Liste ->fetch()) {
                      ?> 
                      <?PHP 
                    }}else { 
                      ?>
                       <p>aucune titre trouvé </p> <?PHP } ?> 
                      </section>
